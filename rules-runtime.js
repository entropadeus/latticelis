...
(function () {
  ...
  const CONDITION_EVALUATORS = {
    ...
    // True when the triggering test's category matches the given department.
    // schema.TEST_CATEGORIES stores title-case labels ('Chemistry', 'Hematology', …),
    // so the rule arg is compared as-is — no enum normalization required.
    'test.department.is': (a, c) => !!c.test && !!a.dept && c.test.category === a.dept,
